...
-----END PGP SIGNATURE-----`;
  ...
  it('Public key encrypted symmetric key packet using symmetric keys', () => {
    ...
    expect(packet.publicKeyAlgorithm).to.equal(100);
  ...
  it('Signature packet using symmetric keys', () => {
    ...
    expect(packet.publicKeyAlgorithm).to.equal(101);
    expect(util.equalsUint8Array(readMAC, expectedMAC));
  });
  ...
  describe('Symmetric key encrypted symmetric key packet', () => {
    ...
    beforeEach(async () => {
      ...
      enc.sessionKey = secret;
      enc.publicKeyAlgorithm = openpgp.enums.publicKey.aead;
      enc.sessionKeyAlgorithm = openpgp.enums.symmetric.aes256;
      enc.publicKeyID.bytes = '12345678';
    });
