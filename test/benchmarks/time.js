...
(async () => {
  ...
  const { armoredKey, privateKey, publicKey, armoredEncryptedMessage, armoredSignedMessage, publicKeyPQC, privateKeyPQC, armoredEncryptedMessagePQC, armoredSignedMessagePQC } = await getTestData();
  ...
  suite.add('openpgp.encrypt pqc', wrapAsync(async () => {
    const message = await openpgp.createMessage({ text: 'plaintext' });
    await openpgp.encrypt({ message, encryptionKeys: publicKeyPQC });
  }));
  ...
  suite.add('openpgp.sign pqc', wrapAsync(async () => {
    const message = await openpgp.createMessage({ text: 'plaintext' });
    await openpgp.sign({ message, signingKeys: privateKeyPQC });
  }));
  ...
  suite.add('openpgp.decrypt pqc', wrapAsync(async () => {
    const message = await openpgp.readMessage({ armoredMessage: armoredEncryptedMessagePQC });
    await openpgp.decrypt({ message, decryptionKeys: privateKeyPQC });
  }));
  ...
  suite.add('openpgp.verify pqc', wrapAsync(async () => {
    const message = await openpgp.readMessage({ armoredMessage: armoredSignedMessagePQC });
    await openpgp.verify({ message, verificationKeys: publicKeyPQC, expectSigned: true });
  }));
...
-----END PGP PRIVATE KEY BLOCK-----`;
  ...
  const armoredKeyPQC = `-----BEGIN PGP PRIVATE KEY BLOCK-----
...
-----END PGP PRIVATE KEY BLOCK-----`;
  ...
  const privateKeyPQC = await openpgp.readKey({ armoredKey: armoredKeyPQC });
  const publicKeyPQC = privateKeyPQC.toPublic();
  ...
  const armoredEncryptedMessagePQC = await openpgp.encrypt({ message: plaintextMessage, encryptionKeys: publicKeyPQC });
  const armoredSignedMessagePQC = await openpgp.sign({ message: await openpgp.createMessage({ text: 'plaintext' }), signingKeys: privateKeyPQC });
  ...
  return {
    ...
    publicKeyPQC,
    privateKeyPQC,
    armoredEncryptedMessagePQC,
    armoredSignedMessagePQC
  };
}
