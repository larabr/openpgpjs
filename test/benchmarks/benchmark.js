...
(async () => {
  ...
  suite.add('openpgp.verify', wrapAsync(async () => {
    ...
    const message2 = await openpgp.readMessage({ armoredMessage: armoredSignedMessage });
    await openpgp.verify({ message: message2, verificationKeys: publicKey, expectSigned: true });
  }));
