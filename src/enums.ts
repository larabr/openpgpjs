...
namespace enums {
   ...
   * @readonly
   */
  export const signatureSubpacket = {
   ...
   * @readonly
   */
  export const reasonForRevocation = {
...
// }
// TODOOOO type ineference here is broken so can just leave `any` (probal is probs that the resulting map is a mix of types [keyof Enum]: Record<K, T[K])
const reversedEnumMap = (<const T extends Record<keyof T, PropertyKey>, const K extends keyof T>() => Object.entries<Record<K, T[K]>>(enums as any).reduce((acc, [enumName, enumType]) => {
  ...
  return acc;
}, new Map<Record<K, T[K]>, Record<T[K], K>>()))() as Map<Enums, Record<EnumTypes[keyof EnumTypes], string/*keyof EnumValues<Enums>*/ >>;
...
/**
 * Typescript magic from https://stackoverflow.com/a/78948819 .
 * This accepts an enum in input and extends/improves its type definition to match
 * the actual runtime behavior, where a reverse-mapping is created pointing from
 * integrer values (not string) to keys
 * (e.g. strongReverseEnumMapping(enums.s2k) -> { 'simple' -> 0, ..., 0 -> 'simple' }).
 *
 * This improved type definition is helpful to achieve type binding between enum type and value
 * in `enums.write` and `.read`,
 * without having to manually declare a function signature overload per enum type.
 * Example of final result:
 *   write(enums.curve, enums.curve.ed25519)
 *   // @ts-expect-error
 ...
 */
export function write<const T extends Enums, const K extends keyof T>(type: T, labelOrValue: K | T[K]) { // this is different than existing behaviour (since strings werent converted
...
/**
 * Converts enum value to the corresponding string label, if it exists.
 * @param {Object} type target enum type
...
export function read<const T extends Enums, const K extends keyof T>(type: T, enumValue: T[K]): K {
  ...
  //   });
  // }
  const reversedEnum = reversedEnumMap.get(type);
  const enumLabel = reversedEnum?.[enumValue as any/* EnumValues<T> */];
  if (enumLabel !== undefined) {
    return enumLabel as K;
  }
...
export type EnumTypes = {
  [K in keyof typeof enums]: EnumValues<typeof enums[K]>
};
